...
void Aquarium::RenderBottom(Shader& shader, const Camera& camera)
{
	...
	// Bottom part (solid)
	glm::mat4 model2 = glm::scale(glm::mat4(1.0f), glm::vec3(10.8f, 4.2f, 13.8f));
	model2 = glm::translate(model2, glm::vec3(1.0f, 0.4f,.2f));
...
{
    glDisable(GL_CULL_FACE); // Disable face culling for double-sided rendering

    shader.use();
    shader.setMat4("projection", camera.GetProjectionMatrix());
    shader.setMat4("view", camera.GetViewMatrix());

    glm::mat4 model1 = glm::scale(glm::mat4(1.0f), glm::vec3(10.8f, 7.2f, 12.0f));
    model1 = glm::translate(model1, glm::vec3(1.0f, 0.42f, 0.25f));

    shader.setMat4("model", model1);
    shader.SetVec3("objectColor", 1.0f, 1.0f, 1.0f); // Blue color
    shader.setFloat("transparency", 0.15f); // Transparency for glass
    glBindVertexArray(VAO1);
    glDrawArrays(GL_TRIANGLES, 0, 30);
    shader.setFloat("transparency", 1.0f); // Reset transparency
...
void Aquarium::RenderWater(Shader& shader, const Camera& camera, float time, const std::string& texturePath)
{
	...
	model = glm::scale(glm::mat4(1.0f), glm::vec3(10.7f, 6.2f, 11.9f));
	model = glm::translate(model, glm::vec3(1.01f, 0.41f, 0.247f));
	

	shader.setMat4("model", model);
	shader.SetVec3("objectColor", 0.0f, 0.0f, 1.0f); // Light blue water
...
void Aquarium::RenderCuboid(Shader& shader, const Camera& camera)
{
	...
	shader.SetVec3("objectColor", 0.96f, 0.87f, 0.70f);

	// Model transformations: translate and scale the cuboid
	glm::mat4 model = glm::mat4(1.0f);
	model = glm::scale(model, glm::vec3(8.93f, 5.2f, 11.8f));
	model = glm::translate(model, glm::vec3(1.215f, .501f, 0.25f)); // Position the cuboid
	shader.setMat4("model", model);
...
void Aquarium::RenderAlgae(Shader& shader, const Camera& camera, float time, glm::vec3 basePosition, int numAlgae, glm::vec3 color) {
	...
	for (int i = 0; i < numAlgae; ++i) {
		...
		model = glm::scale(model, glm::vec3(.15f, .6f, .15f)); // Small algae size
